...
class PointMAE(pl.LightningModule):
    def __init__(
        ...
        self.increase_dim = nn.Sequential(
            nn.Conv1d(encoder_dim, num_channels*encoder_dim, 1),
        )

        self.do_koleo = koleo_weight > 0
    ...
    def forward(
        ...
        total_embeddings = (encoder_output_embeddings * unmasked.unsqueeze(-1)
        ...
        koleo_loss = 0
        if self.do_koleo:
            koleo_loss = self.koleo_loss(masked_output_embeddings)
        ...
        return chamfer_loss, koleo_loss, ae_loss
    ...
    def training_step(self, batch, batch_idx: int) -> torch.Tensor:
        ...
        chamfer_loss, koleo_loss, ae_loss = self._perform_step(points, lengths)

        self.log("train_loss_chamfer", chamfer_loss, on_epoch=True, sync_dist=True)
        if self.do_koleo:
            self.log("train_loss_koleo", self.hparams.koleo_weight * koleo_loss, on_epoch=True, sync_dist=True)
        if self.do_ae:
            self.log("train_loss_ae", self.hparams.ae_weight * ae_loss, on_epoch=True, sync_dist=True)

        loss = chamfer_loss + self.hparams.koleo_weight * koleo_loss + self.hparams.ae_weight * ae_loss
    ...
    def validation_step(
        ...
        self.log("val_loss_chamfer", chamfer_loss, sync_dist=True)

        if self.do_koleo:
            self.log("val_loss_koleo", self.hparams.koleo_weight * koleo_loss, sync_dist=True)
        if self.do_ae:
            self.log("val_loss_ae", self.hparams.ae_weight * ae_loss, sync_dist=True)
        ...
        def xy(dataloader):
            ...
            for i, (data, lengths, labels_batch, _) in enumerate(dataloader):
                ...
                with torch.no_grad():
                    ...
                    pos = self.center_encoding(centers)
                    x = self.encoder(tokens, pos, mask).last_hidden_state.reshape(-1, 384)
